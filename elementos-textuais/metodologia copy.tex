...
\input{elementos-textuais/metodologia-especifica}
% END ADD

\section{Tipo e Abordagem da Pesquisa}
\label{sec:metodologia-1}
A pesquisa adotada é computacional, uma vez que visa usar modelos matemáticos, simulações e algoritmos visando uma análise detalhada de desempenho da plataforma experimental desenvolvida para aplicar no sistema de freio veicular com base em simulações.


\section{Justificativa das Escolhas Metodológicas}
\label{sec:metodologia-2}
As escolhas metodológicas adotadas neste trabalho foram fundamentadas em referências da literatura e na necessidade de validar experimentalmente estratégias de controle digital para sistemas de freio automotivo. A seleção dos componentes, algoritmos e procedimentos buscou responder certas lacunas identificadas em projetos anteriores, garantindo rigor científico e relevância para futuras pesquisas.
...
O bloco hidráulico incorpora 12 válvulas solenoides que abrem ou fecham os circuitos hidráulicos entre o cilindro mestre (1) e os freios (4), como mostra a Figura~\ref{fig:principio-do-modulador-hidraulico-com-valvulas-solenoides}. 
...
O processo de modulação hidráulica utilizado em estratégia ABS possui 3 fases: na primeira fase, o condutor gera uma primeira pressão no Cilindro Mestre (CM) ao pisar no pedal de freio, o resultado dessa pressão aciona diretamente os freios: Traseiro Esquerdo (TE), Dianteiro Direito (DD), Dianteiro Esquerdo (DE), Traseiro Direito (TD), devido à Válvula de Entrada (VE) ser normalmente aberta e a Válvula de Saída (VS) normalmente fechada. 

Nesse cenário, caso ocorra o travamento de uma das rodas, inicia-se a segunda fase do controle, na qual a pressão é reduzida por meio da abertura da Válvula de Saída (VS), direcionando o fluido hidráulico para o Reservatório de Baixa Pressão (RBP), compartimento responsável pelo armazenamento temporário do fluido do sistema. Em seguida, ocorre a fase de recomposição da pressão hidráulica, na qual a Bomba de Retorno (BR) é acionada pelo Motor da Bomba (M), promovendo o retorno do fluido ao circuito de freio. Esse processo resulta em uma diminuição momentânea da força transmitida ao pedal, permitindo ao condutor perceber o seu movimento característico. A Válvula de Alta Pressão (VAP) e a Válvula de Comutação (VC) permanecem no mesmo estado, uma vez que não desempenham função ativa nesse cenário específico.

\subsection{Motor da Bomba de Pressão}
\label{sec:metodologia-4-2}
A função da bomba é manter a pressão hidráulica necessária para modular a força de freio aplicada a cada roda. Isso ajuda a impedir o travamento da roda durante a frenagem e melhora a estabilidade do veículo durante curvas repentinas.
...
A bomba utilizada em sistemas ABS ou ESP não é do tipo engrenagem, mas sim uma bomba de pistão alternativo acionada por um motor elétrico, normalmente do tipo BLDC (\textit{Brushless Direct Current}). Esse motor aciona um mecanismo excêntrico que movimenta o pistão da bomba, permitindo recircular o fluido e restabelecer a pressão no circuito. 

O elemento responsável por armazenar fluido sob pressão — frequentemente um acumulador hidráulico, que pode ser do tipo pistão ou diafragma — não deve ser confundido com a bomba. Quando o sistema ABS ou ESP detecta uma potencial perda de tração, ou iminência de travamento da roda, a bomba é acionada para recompor a pressão rapidamente, garantindo o desempenho de frenagem e a estabilidade do veículo.


\subsection{Sensor da Roda}
\label{sec:metodologia-4-3}

Os sensores de velocidade da roda são usados para medir a velocidade de rotação das rodas do veículo. Os sinais de velocidade são transmitidos por meio de cabos para a placa, que controla a força de frenagem individualmente em cada roda. Esse sinal impede que as rodas travem, outro fator importante de acordo com \citeonline{Konrad2014} é que o sistema de navegação também usam os sinais de velocidade da roda para calcular a distância percorrida (por exemplo, em túneis). Normalmente encontramos os sensores de velocidade da roda em 2 modelos: passivos e ativos. 
...
O sistema de freio do veículo é complexo, de modo que concentra 12 válvulas (mesmo que será abordado durante o desenvolvimento da pesquisa),  onde 2 dessas válvulas são responsáveis por comutar alta pressão com diferenciais mais altos, maior que 0,1 MPa \cite{Konrad2014}. 
Também vale ressaltar que, em uma das estratégias, o Programa de Estabilidade Eletrônica (ESP), é necessário mediante o aumento da pressão para a estabilidade do veículo. 

Em veículos com ESP um sensor de pressão é instalado exclusivamente para monitorar a pressão do óleo no cilindro (9) da Figura~\ref{fig:bloco-do-sistema-de-freio} que tem a finalidade de detectar a pressão aplicada pelo condutor do veículo no cilindro mestre, o que ajuda na estratégia de controle parcialmente ativo.

Em uma estratégia ACC é obrigatório gerar uma pressão, e o sensor de óleo é o responsável por fazer o fechamento da malha de controle.
...
As simulações serão conduzidas no ambiente MATLAB/Simulink, utilizando modelos matemáticos formulados a partir de equações diferenciais ordinárias e representados no formalismo de espaço de estados. Parâmetros como massa do veículo, pressão hidráulica, frequência de modulação por largura de pulso (PWM) e características dos atuadores são ajustados visando avaliar o desempenho do sistema sob diferentes condições operacionais.
